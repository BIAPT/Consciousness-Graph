...
% Participant Path (will need to iterate at the end over all the files)
%create loop for participants if you need
for s = 1:length(step_size)
    ...
    %for p = 1:length(participants)
       ...
       for e = 1:length(states)
            ...
            %% Load data
            recording = load_set(raw_data_filename,data_location); %error here
            ...
            recording = filter_data(recording, recording.data, frequency_band);
            ...
            result.aec = (aec + permute(aec,[2,1,3]))/2; % orthogonilization?
            ...
            aec_state_filename = strcat(participant_out_path, filesep,state, '_aec.mat'); 
            save(aec_state_filename, 'result');
            
            %reorder channels
            [r_aec, r_labels, r_regions, r_location] = reorder_channels(result.aec, result.labels,'electrodes.csv');

            plot_wpli(r_aec,strcat(participant," ",session," ",state," ",eyes," Whole Brain AEC"),[],'jet',0); 
            colorbar
            imagepath = strcat(participant_out_path,filesep,state,'_whole_aec.fig');
...
%% CREATE SLIDING WINDOW will slice up the data into windows and return them
    ...
    iterator = 1:step_size:(length_data - window_size);
    num_window = length(iterator);
    
    windowed_data = zeros(num_window, num_region, window_size);
    index = 1;
    for i = 1:step_size:(length_data - window_size)
        windowed_data(index,:,:) = data(:, i:i+window_size-1);%index out of bounds error here
        index = index + 1;
    end
